...
pub fn handle_archive_command() -> Result<(), TaskError> {
    let tasks = load_tasks()?; // Changed from `let mut tasks`

    let mut incomplete_tasks = Vec::new();
    let mut completed_tasks = Vec::new();

    for task in tasks { // This loop takes ownership of each `task`
        if task.completed {
    ...
    // Save only incomplete tasks back to the main file
